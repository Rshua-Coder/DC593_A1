...
/* Delete a file from the FTP server. This is fairly straightforward.
   Hostname is the name/IP address of the server. username is the
   username used to connect to the server with. Passwd is the password
   used to authentificate the username. filename is the name of the file
   to delete. It should be the full pathname of the file. ftp_printf is
   a function to be called to perform printing. On success 0 is
   returned. On error a negative value is returned indicating the type
   of error. */
...
/* Read file list in input directory */
int ftp_list(char * hostname,
            char * username,
            char * passwd,
            char * filename,
            char * buf,
