...
#ifdef CYGHWR_HAL_MIPS_DSP
    // There are common to all saved states too
    // DSP only register
    CYG_HAL_MIPS_REG    ac1_hi;
    CYG_HAL_MIPS_REG    ac1_lo;
    CYG_HAL_MIPS_REG    ac2_hi;
    CYG_HAL_MIPS_REG    ac2_lo;
    CYG_HAL_MIPS_REG    ac3_hi;
    CYG_HAL_MIPS_REG    ac3_lo;
    CYG_WORD32          dspctrl;
#endif
} HAL_SavedRegisters;
...
// The following code should be placed between
//    (_regs_)->d[04] = (CYG_HAL_MIPS_REG)(_thread_);
// and
//    (_regs_)->d[30] = (CYG_HAL_MIPS_REG)(_sp_);

#if 0

    (_regs_)->lo = 0;                                 /* LO = 0                 */      \
    (_regs_)->hi = 0;                                 /* HI = 0                 */      \
    (_regs_)->ac1_lo = 0;                             /* AC1 LO = 0             */      \
    (_regs_)->ac1_hi = 0;                             /* AC1 HI = 0             */      \
    (_regs_)->ac2_lo = 0;                             /* AC1 LO = 0             */      \
    (_regs_)->ac2_hi = 0;                             /* AC1 HI = 0             */      \
    (_regs_)->ac3_lo = 0;                             /* AC1 LO = 0             */      \
    (_regs_)->ac3_hi = 0;                             /* AC1 HI = 0             */      \
    (_regs_)->dspctrl = 0;                            /* DSPControl = 0         */      \
...
#define CYGNUM_HAL_GEN_REG_SIZE         ((32 + 10) * (CYG_HAL_MIPS_REG_SIZE))

// FPU register size
#ifdef CYGHWR_HAL_MIPS_FPU
    #define CYGNUM_HAL_FPU_REG_SIZE     ((32 + 2) * (CYG_HAL_MIPS_REG_SIZE))
#else
    #define CYGNUM_HAL_FPU_REG_SIZE     0
#endif

// DSP register size
#ifdef CYGHWR_HAL_MIPS_DSP
    #define CYGNUM_HAL_DSP_REG_SIZE     (6 * (CYG_HAL_MIPS_REG_SIZE) + 4)
#else
    #define CYGNUM_HAL_DSP_REG_SIZE     0
#endif

#define CYGNUM_HAL_STACK_CONTEXT_SIZE   (CYGNUM_HAL_GEN_REG_SIZE + CYGNUM_HAL_FPU_REG_SIZE + CYGNUM_HAL_DSP_REG_SIZE)
...
#if defined(CYGPKG_HAL_MIPS_TLB_SUPPORT)
#define CYGARC_KSEG_MASK                               (0xE0000000)
#define CYGARC_KSEG_CACHED                             (0x00000000)
#define CYGARC_KSEG_UNCACHED                           (0x20000000)
#define CYGARC_KSEG_CACHED_BASE                        (0x00000000)
#define CYGARC_KSEG_UNCACHED_BASE                      (0x20000000)
#else
#define CYGARC_KSEG_MASK                               (0xE0000000)
