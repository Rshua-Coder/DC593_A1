...
#define M_DIR_NAME      (1 << 0)
#define M_FS_NAME       (1 << 1)
#define M_DEV_NAME      (1 << 2)
...
struct cyg_mtab_entry
{
    ...
    int                 m_flag;         // memory allocated by us
} CYG_HAL_TABLE_TYPE;
...
struct CYG_UIO_TAG
{
    ...
    off64_t       	 uio_offset;	/* offset into file this uio corresponds to */
...
typedef off64_t cyg_fileop_lseek   (struct CYG_FILE_TAG *fp, off64_t *pos, int whence );
...
struct CYG_FILE_TAG
{
    ...
    off64_t       	        f_offset;       /* current offset               */
...
__externC void cyg_set_timestamp(time_t new_value);
