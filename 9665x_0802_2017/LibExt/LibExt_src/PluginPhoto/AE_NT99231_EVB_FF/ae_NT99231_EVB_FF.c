...
UINT32 AE_CUR_Y;
UINT32 AE_DARK_Y_TOP = AE_EXPECT_Y_NIGHT;
UINT32 AE_DARK_Y_BOT = (AE_EXPECT_Y_NIGHT*3) >> 2;
...
//Arg2 + IFE
    ...
    AE_setExpT(&ExpTSetting);
    //DBG_ERR("LumY = %d\r\n", AeInfo.LumY);
    if(PrvNewAeArg.ISOGain <= 300)
    ISORatio = 0;
    else if(PrvNewAeArg.ISOGain >= 600)
    ISORatio = 256;
    else
        ISORatio = 256 * (PrvNewAeArg.ISOGain - 300) / 300;
    //DBG_ERR("ISO = %d, ratio = %d\r\n", PrvNewAeArg.ISOGain, ISORatio);
    AE_setGain(&Gain);
...
#if 0
    {
        char pString[512];
        SenFP_ClearOSD();
        sprintf(pString, "%3d %3d", PrvNewAeArg.ISOGain, PrvNewAeArg.ExpoTime);
        SenFP_ShowOSDString(pString, 20, 40, 4);
    }
#endif
}
