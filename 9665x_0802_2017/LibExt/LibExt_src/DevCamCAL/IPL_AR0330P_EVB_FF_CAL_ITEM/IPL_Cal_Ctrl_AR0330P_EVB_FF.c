...
_INLINE UINT32 _SIE_IntCB(ENG_SUBFUNC *Info,IPL_HAL_GROUP* group)
{
    ...
    */
//sun 0610
    Info->pSie_sub->OBSub.Offset=0xa0;
    ...
    //ecs
    SetCalSensorMode(IPL_AlgGetUIInfo(IPL_SEL_PRVSENMODE));
...
_INLINE void _SIEIn(SENSOR_INFO *pSenInfo, Cal_SENSOR_INFO *pCalSenInfo,SIE_WINDOW *Win)
{
    ...
    if (pSenInfo->Mode->TransHD[0].DataStart != 0)
    {
        CropShift_X += (pSenInfo->Mode->TransHD[0].DataStart + (4 - (pSenInfo->Mode->TransHD[0].DataStart % 4)))*2;
    }
    if (pSenInfo->Mode->TransVD[0].DataStart != 0)
    {
        CropShift_Y += (pSenInfo->Mode->TransVD[0].DataStart + (4 - (pSenInfo->Mode->TransVD[0].DataStart % 4)))*2;
    }
    ...
    else
    {
        ...
        Win->Crop.StartPix.x = Win->Crop.StartPix.x;
        //DBG_ERR("start %d = (active %d - crop %d)/2\r\n", Win->Crop.StartPix.x, Win->Act.SizeH, Win->Crop.SizeH);
...
static void Prv_CtrlInit(pIPLC_INFO pIplObj,pCTRL_PTR pCtrlptr)
{
    ...
    if (0)//(CalApi()->fpGetCalUsbMode())
    {
        UINT32 PC_size_w = 0;
        UINT32 PC_size_h = 0;
        CalUSB_GetDispSize(&PC_size_w, &PC_size_h);
        pIplObj->pBaseInfo->IME_P2_PPB0.Ch[0].Width = PC_size_w;
        pIplObj->pBaseInfo->IME_P2_PPB0.Ch[0].Height = PC_size_h;
    }
    else
    {
...
static void DP_CtrlInit(pIPLC_INFO pIplObj,pCTRL_PTR pCtrlptr)
{
    ...
    // OB sun0610
    pIplObj->psie->pSubFunc->OBSub.Offset=0xa0;
