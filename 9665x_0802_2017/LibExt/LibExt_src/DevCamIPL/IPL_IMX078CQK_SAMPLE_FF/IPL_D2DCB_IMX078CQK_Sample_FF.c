...
static UINT32 IME_CAP_D2D_CB(IME_SUBFUNC* subF, IME_UPDATE *Update, UINT32 eventID,fpRegDump *fpRegD)
{
    ...
    if ((eventID & (~IPL_FLOW_RESERVE)) == IPL_FLOW_IMED2D_QV)
        IQCB_SettingFlowFunc(IQCB_D2D_IME_QV, &Info, &group);
    if ((eventID & (~IPL_FLOW_RESERVE))== IPL_FLOW_IMED2D_QVREF)
...
static UINT32 PREIME_CAP_D2D_DCECB(USIZE *pRef_1x,Coordinate *pRef_1xCen,DCE_SUBFUNC* subF, DCE_UPDATE *Update, UINT32 eventID,fpRegDump *fpRegD)
{
    ...
    switch(eventID)
    {
        case (IPL_FLOW_RAW2YCC_QV|IPL_FLOW_Y|IPL_FLOW_U|IPL_FLOW_V):
            ...
            //pRef_1x->w = IPL_UTI_IPESUB_ADJ_HSIZE(tmpSizeH, IPL_CtrlGetInfor(IPL_ID_1, IPLCTRL_CAPQVREFRATIO), 4);
            //pRef_1x->h = IPL_UTI_IPESUB_ADJ_VSIZE(tmpSizeV, IPL_CtrlGetInfor(IPL_ID_1, IPLCTRL_CAPQVREFRATIO), 4);
            pRef_1x->w = tmpSizeH;
            pRef_1x->h = tmpSizeV;
            //ethan test
