#ifndef _IQS2_SETTINGFLOW_AR0330M_C12_C
#define _IQS2_SETTINGFLOW_AR0330M_C12_C
...
SceneRender_RTXGainInfo GetRtxGainInfo = {1024, 1024, 1024, 1024,1024, 1024, 1024, 0};
...
BOOL Gamma_update = FALSE;
UINT8 updategamma = 0;
///////////////////////////////////////////////
// Body
///////////////////////////////////////////////
static void set_3180(void)
{
    SENSOR_CMD Cmd={0,0,{0,0}};
    Cmd.uiAddr = 0x3180;
    Sensor_ReadReg(0,&Cmd);
    //DBG_ERR("%x \r\n",Cmd.uiData[0]);
    Cmd.uiData[0] = Cmd.uiData[0]|0x2000;
    //DBG_ERR("%x \r\n",Cmd.uiData[0]);
    Sensor_WriteReg(0, &Cmd);
}
...
void IQS_Gamma_Intpl(UINT32* G1, UINT32* G2, INT32 X1, INT32 X2, INT32 X, UINT32 *G)
{
    int i;
    for ( i=0; i<130; i++ )
        G[i] = IQS_Intpl((int)X, (int)G1[i],  (int)G2[i], (int)X1, (int)X2);
}
...
void IQS_CC_Intpl(UINT16* CC1, UINT16* CC2, INT32 X1, INT32 X2, INT32 X, UINT16 *CC)
{
    ...
    for ( i=0; i<9; i++ )
    {
        ...
        if ( E2 > 0x400 )
            E2 = E2 - 0x800;
        CC[i] = IQS_Intpl((int)X, (int)E1,  (int)E2, (int)X1, (int)X2);
    }
}
...
static ER SIE_IQparam(IQS_FLOW_MSG MsgID, SIE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    UINT32 ISO, ExpT, Iris;
    UINT16 ob;
  //  subf->sieFuncEn |=OBSubSelEn;
    group->sieUpdate |= SIE_OBOFS_;

    AE_GetCapAEArg(&ISO, &ExpT, &Iris);

    ob = IQS_Intpl((int)ISO, 0xa8, 0xa0, 150, 1000);

    subf->OBSub.Offset=ob;
...
extern UINT32 IQS_GammaLut_Night[130];
static ER IPE_IQparam(IQS_FLOW_MSG MsgID, IPE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    if ( IQS_GetIsoRange( MsgID, &iso, &isoL, &isoH, &ISO_IdxL, &ISO_IdxH ) == TRUE )
    {
        memcpy(&IQ_IPE_GBal, &(IPE_Gbal[ISO_Value]), sizeof(IPE_GBAL));
        IQ_IPE_GBal.Thres = IQS_Intpl((int)iso, (int)IPE_Gbal[ISO_IdxL].Thres, (int)IPE_Gbal[ISO_IdxH].Thres, (int)isoL, (int)isoH);
    ...
    switch((UINT32)MsgID)
    {
        ...
        case IQCB_PREIME_D2D_IPE_QV:
            ...
            if ( MsgID == IQCB_PRV_IPE_INIT || MsgID == IQCB_PREIME_D2D_IPE_PRI || MsgID == IQCB_PREIME_D2D_IPE_QV || MsgID == IQS_MANUAL_TRIG )
            {
                ...
                memcpy(IQS_GammaLut, GammaLUT_128Tab, sizeof(UINT32)*130);
                Gamma_update = TRUE;
            }
            if ( MsgID == IQS_AE_END && updategamma == 1)
            {
                group->ipeUpdate |= ( IPE_RGBGAMMA_ );
                updategamma = 0;
            }

            if(IPE_RGBGAMMA_== (group->ipeUpdate&IPE_RGBGAMMA_) )
              {
                static UINT32 aLv_old;
                UINT32 aLv = AE_GetPrvEVValue();
                #define IQS_LV_DAY_END     100
                #define IQS_LV_NIGHT_START 60
                if( (aLv != aLv_old) && !((aLv > IQS_LV_DAY_END) && (aLv_old > IQS_LV_DAY_END)) && !((aLv < IQS_LV_NIGHT_START) && (aLv_old < IQS_LV_NIGHT_START)) )
                    Gamma_update = TRUE;
                ...
                {
                    IQS_CC_Intpl( Normal_CC_night, Normal_Day, IQS_LV_NIGHT_START, IQS_LV_DAY_END, aLv, Normal_CC);
                    IQS_Gamma_Intpl(IQS_GammaLut_Night, GammaLUT_128Tab, IQS_LV_NIGHT_START, IQS_LV_DAY_END, aLv, IQS_GammaLut);
            ...
            //////////////////
            if(Gamma_update == TRUE){
                IQ_IPE_RGBGAMMA.GammaTbl_TblAddr = (UINT32)IPE_GammaReorgon((UINT32 *)IQS_GammaLut, (UINT32 *)IQS_GammaLut, (UINT32 *)IQS_GammaLut);
                Gamma_update = FALSE;
            }
...
static ER IQS_SIE_IQparam(IQS_FLOW_MSG MsgID)
{
    ...
    IPH_ENG_Read(IPL_ID_1,IPL_HAL_SIE, idx, readBit, (void*)&sieSubf.sieFuncEn);

    ret = SIE_IQparam(MsgID, &sieSubf, &group);
    if ( ret != E_OK )
    {
        SIG_IFESEM;
        return ret;
    ...
    Ipc_IQ.EngSubF.pSie_sub = &sieSubf;
    Ipc_IQ.pGrp = &group;
    Ipc_IQ.Idx = idx;
    IPL_CTRLRunTimeChg_IQ(IPC_SIE_IQparam,(void*)&Ipc_IQ);
    SIG_IFESEM;
    return E_OK;
    return E_OK;
}
