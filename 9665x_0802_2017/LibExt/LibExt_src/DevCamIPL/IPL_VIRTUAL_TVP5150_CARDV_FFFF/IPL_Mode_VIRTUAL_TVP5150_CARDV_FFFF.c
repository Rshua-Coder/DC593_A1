...
UINT32 CheckTVP150_InputMode(void)
{
    SENSOR_CMD Cmd;
    UINT32 ER;

    Cmd.uiAddr = 0x8c;
    ER = Sensor_ReadReg(IPL_ID_2, &Cmd);

    if(Cmd.uiData[0] == 0x83)
                                    ...
                                    IPL_ISR_OBJ_INFO *IsrObjInfo, IPC_INFO *IpcInfo, AE_OBJ_INFO *AeObj, AWB_OBJ_INFO *AwbObj, AF_OBJ_INFO *AfObj, UINT32 RevDZFlag)
{
    ...
    //if(Cmd.uiData[0] == 0x83) // PAL
    if(CheckTVP150_InputMode() == SENSOR_MODE_2) // PAL
        IPL_AlgSetUIInfo2(Id, IPL_SEL_PRVSENMODE, SENSOR_MODE_2);
    else // NTSC
