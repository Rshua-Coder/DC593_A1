...
void IPL_SIESetOB_FCB(UINT32 Id, SieFuncEn *FuncEn, SIE_INTE_EN *InteEn, SIE_UPDATE *Update, SIE_OBAVG *OBAvg, SIE_OBSUB *OBSub)
{
    ...
    {
        OBSub->Offset = 0X3A;  //iso100: 0xa0, iso1600: 0x90
        OBSub->DtGain = 0x20;
        *FuncEn |= OBSubSelEn;
...
{
    if (0)//(IPL_CtrlGetInfor(Id, IPLCTRL_SIE_ECS_EN) == ENABLE)
    {
        InitEcsTbl();
...
BOOL IPL_ChkImePathInfo(UINT32 Id, IPL_IME_PATH PathId, IPL_IME_PATH_INFO *Info)
{
    ...
    else if (PathId == IPL_IME_PATH3)
    {
        ...
        IPCL_IMEP3_CROP_INFO CropInfo = IPC_GetIMECropInfo();

        if (CropInfo.CropEn == ENABLE)
        {
            //new
            newInfo.type = Info->ImgFmt;
            newInfo.Ch[IPL_IMG_YUV_Y].Width = CropInfo.Crop_Size_H;
            newInfo.Ch[IPL_IMG_YUV_Y].LineOfs = CropInfo.Crop_Size_H;
            newInfo.Ch[IPL_IMG_YUV_Y].Height = CropInfo.Crop_Size_V;
            newInfo.Ch[IPL_IMG_YUV_UV] = IPL_UTI_Y_INFO_CONV2(newInfo.type, newInfo.Ch[IPL_IMG_YUV_Y]);
        }
        else
        {
