/**
    Ctrl fw OV8858_EVB_FF.

    @file       IPL_CtrlOV8858_EVB_FFFW.c
    @ingroup    mISYSAlg
...
void IPL_CtrlPrvInitFlowC(UINT32 Id, IPLCTRL_INFO *CtrlInfo)
{
    ...
    if (IPL_CtrlGetInfor(Id, IPLCTRL_IME_P3_EN) == ENABLE)
    {
        ...
        {
            if ((CtrlInfo->ChgMode->BufSizeH_P3 * CtrlInfo->ChgMode->BufSizeV_P3) < (CropInfo.Crop_Size_H * CropInfo.Crop_Size_V))
            {
            ...
            else
            {
                ...
                IPL_CtrlSetInfor(Id, IPLCTRL_IME_P3_ORG_SIZE_HLOS, CropInfo.Crop_Size_H);
