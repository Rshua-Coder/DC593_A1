...
SIE_EmbeddedCS sm_Ecs = {ECS_D2F8,DISABLE,DISABLE,_BIT_0_1};
...
static ER SIE_IQparam(IQS_FLOW_MSG MsgID, SIE_SUBFUNC *subf,IPL_HAL_GROUP* group)
{
    ...
    switch((UINT32)MsgID)
    {
            ...
            subf->Ecs.pecs = &sm_Ecs;
            subf->Ecs.Inaddr = (UINT32)IQS_ECSTab;
			...
			{
			    UINT32 Effect_Value = IPL_AlgGetUIInfo(IPL_SEL_IMAGEEFFECT);

            	subf->Ecs.pecs = &sm_Ecs;
				if (Effect_Value== SEL_IMAGEEFFECT_BW)
				{DBG_ERR("off\r\n");
					subf->Ecs.Inaddr = (UINT32)IQS_ECSTab_init;//(UINT32)	IQS_ECSTab_init;
					//subf->sieFuncEn &= (~ECSEn);
