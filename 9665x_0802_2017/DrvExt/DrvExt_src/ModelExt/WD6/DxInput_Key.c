...
static UINT32 DrvKey_DetGsensorEvent(void)
{
    ...
    if(bGSensorStatus==TRUE)
    {
        ...
        //debug_msg("****** KEY FLGKEY_CUSTOM1 *******\r\n");
    }

    return uiKeyCode;
}

static UINT32 g_uiACUnPlugCnt = 0;
static BOOL g_bACUnPlugStd = FALSE;
void DrvKey_SetACUnPlugStd(BOOL en)
{
	g_bACUnPlugStd = en;
}
static UINT32 DrvKey_DetACUnPlugEvent(void)
{
	UINT32 uiKeyCode = 0;

	if (g_bACUnPlugStd == TRUE)
	{
		g_uiACUnPlugCnt++;
	}
	else
	...
	//debug_msg("g_uiACUnPlugCnt = %d\r\n",g_uiACUnPlugCnt);

	if (g_uiACUnPlugCnt >= 670)	// 15 Sec Power down
	{
		uiKeyCode = FLGKEY_KEY_POWER;
		if(g_uiACUnPlugCnt > 672)
		{
			g_bACUnPlugStd = FALSE;
			g_uiACUnPlugCnt = 0;
		}
	}
	return uiKeyCode;
...
UINT32 DrvKey_DetNormalKey(void)
{
	...
	if(!gpio_getPin(GPIO_KEY_DOWN))
	{
		...
		//debug_msg("****** ModeTimerCount = %d ******\r\n",ModeTimerCount);
		if(ModeTimerCount > 60)//100
		{
			uiKeyCode |= FLGKEY_LONGMODE;
			if(ModeTimerCount > 62)//102
			{
