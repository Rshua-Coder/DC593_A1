...
#if (LED_MOVIE_CTRL == LED_MOVIE_BY_GPIO)
        gpio_clearPin(GPIO_MOVIE_LED);
#endif
    break;


    case GPIOMAP_LED_NIGHT:
#if (LED_NIGHT_CTRL == LED_NIGHT_BY_GPIO)
        gpio_setPin(GPIO_NIGHT_LED);
#endif
    break;
#endif
...
BOOL LED_IsLEDOn(UINT32 uiLED)
{
    ...
    case GPIOMAP_LED_NIGHT:
#if (LED_NIGHT_CTRL == LED_NIGHT_BY_GPIO)
        bvalue = gpio_getPin(GPIO_NIGHT_LED);
#endif
    break;
#endif
