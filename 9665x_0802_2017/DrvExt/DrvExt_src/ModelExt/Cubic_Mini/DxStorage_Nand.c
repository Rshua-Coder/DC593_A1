...
// Fill in timing parameters for SPI flash.
static BOOL FM25Q32_identifySpi(UINT32 uiMfgID, UINT32 uiTypeID, UINT32 uiCapacity, PSPIFLASH_IDENTIFY pIdentify)
{
    //FM25Q32 (4MBytes)
    DBG_IND("=====SPI flash uiMfgID=%d, uiTypeID=%d, uiCapacity=%d\r\n", uiMfgID, uiTypeID, uiCapacity);
    if ((uiMfgID == 0xA1) && (uiTypeID == 0x40) && (uiCapacity == 0x16))
    {
        ...
        pIdentify->uiSectorEraseTime = 300;
...
UINT32 DrvNANDGetcaps0(UINT32 CapID, UINT32 Param1) // Get Capability Flag (Base on interface version)
{
    ...
    case STORAGE_CAPS_HANDLE:
        ...
        #else
        pStrgObj = spiflash_getStorageObject(STRG_OBJ_FW_RSV1);
        if (pStrgObj == NULL)
        {
            DBG_ERR("No FW RSV1 object\r\n");
        }

        #if 1 // Enable Fast Flash access - 80Mhz.
        spi_setConfig(SPI_ID_1, SPI_CONFIG_ID_FREQ, 80000000);
        #endif

        pStrgObj->ExtIOCtrl(STRG_EXT_CMD_SPI_IDENTIFY_CB, (UINT32)FM25Q32_identifySpi, 0);
        v = (UINT32)pStrgObj;
        #endif
        break;
    default:
