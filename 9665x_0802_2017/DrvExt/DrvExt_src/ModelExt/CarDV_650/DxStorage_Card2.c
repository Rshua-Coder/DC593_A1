...
UINT32 DrvCARD2Init(void* pInitParam) // Set Init Parameters
{
    ...
    //#NT#2013/07/22#KS Hung -begin
    sdio2_setConfig(SDIO_CONFIG_ID_DIS_CMD17_RANGE_CHECK, TRUE);
    //#NT#2013/07/22#KS Hung -end
    /*
...
#define KEYSCAN_CARD2DET_GPIO_INT  GPIO_INT_00
#define KEYSCAN_CARD2DET_INT       DISABLE
#define KEYSCAN_CARD2WRPTDET_GPIO  DISABLE
...
#if (KEYSCAN_CARD2DET_INT == ENABLE)
static volatile UINT32  uiStrgCard2IntCnt  = 0;
...
/**
  ...
  @return void
*/
static void KeyScan_DetStrgCard2Isr(UINT32 uiEvent)
{
    uiStrgCard2IntCnt++;

    // Debounce
    if (uiStrgCard2IntCnt > 1)
    {
        uiStrgCard2IntCnt = 0;
    }
}
...
void DrvCARD2_SetInsert(UINT32 status)
{
    #if (KEYSCAN_CARD2DET_INT == ENABLE)
    if(status)
        gpio_setIntTypePol(KEYSCAN_CARD2DET_GPIO_INT, GPIO_INTTYPE_LEVEL, GPIO_INTPOL_POSHIGH);
    else
        gpio_setIntTypePol(KEYSCAN_CARD2DET_GPIO_INT, GPIO_INTTYPE_LEVEL, GPIO_INTPOL_NEGLOW);
    #endif
}
...
{
    #if (KEYSCAN_CARD2DET_INT == ENABLE)
    gpio_setIntIsr(KEYSCAN_CARD2DET_GPIO_INT, KeyScan_DetStrgCard2Isr);
    gpio_enableInt(KEYSCAN_CARD2DET_GPIO_INT);
    #endif
}
...
{
#if 1
    return (gpio_getPin(GPIO_CARD2_DETECT) == 0 ? TRUE : FALSE);
#else
    return TRUE;
#endif
}
...
{
#if 0
    return (gpio_getPin(GPIO_CARD2_WP) == 1 ? TRUE : FALSE );
#else
    return FALSE;
#endif
}
