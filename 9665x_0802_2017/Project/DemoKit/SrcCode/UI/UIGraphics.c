...
void UI_DrawOsdString(char *str, UINT32 x, UINT32 y, UINT32 uiColor, BOOL bClear)
{
    ...
    if (bClear)
    {
        ...
        GxGfx_FillRect(pDCList[GxGfx_OSD], x, y, uiWidth, y+26);
    }
...
void DCT_UI_Screen_Rotate(void) //drh 20170927
{
	 DC** pDCList;
    UINT32 uiWidth;
	View_Window_ConfigAttr(0,2);	
    pDCList = (DC**)UI_BeginScreen();	
	UI_EndScreen((UINT32)pDCList);
}

void UI_ShowStringByColor(char *pStr, PIRECT pRect, UINT32 uiFrColor, UINT32 uiBgColor)
{
	DC** pDCList;
...
void UI_FillRect(PIRECT pRect, UINT32 uiBgColor)
{
	DC** pDCList;
