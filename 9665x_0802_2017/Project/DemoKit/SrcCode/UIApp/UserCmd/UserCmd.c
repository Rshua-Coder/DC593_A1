...
USERCMD_ITEM g_UserCmdItem[] =
{
    ...
    {USERCMD_GPSTEST,       UserCmd_GPSTest},
    //{USERCMD_MAX,           NULL},
};
    ...
    else
    {
        while(uiFileIdx < lSize)
        {
            ...
            if(uiCmdIdxEnd > uiCmdIdxStart)
            {
                ...
                {
                    if((uiCmdIdxEnd - uiCmdIdxStart + 1) <= MAX_CMD_SCRIPT_FILE_LEN)
                    {
...
//-------------------------------------------------------------------------------------------------
void UserCmd_WavTest(void)
{
    Ux_CloseWindow((VControl *)(&UIFlowWndMovieCtrl), 0);
    Ux_OpenWindow((VControl *)(&WavTestWndCtrl), 0);
}
...
//-------------------------------------------------------------------------------------------------
void UserCmd_GPSTest(void)
{
    Ux_CloseWindow((VControl *)(&UIFlowWndMovieCtrl), 0);
    Ux_OpenWindow((VControl *)(&GPSTestWndCtrl), 0);
}
