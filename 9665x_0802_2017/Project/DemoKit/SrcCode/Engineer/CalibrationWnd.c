...
#define CALIBRATE_STRING_AREA_NUMBER      12
...
INT32 CalibrationWnd_OnOpen(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    Input_SetKeyMask(KEY_PRESS, FLGKEY_UP|FLGKEY_DOWN|FLGKEY_LEFT|FLGKEY_ENTER);
    Input_SetKeyMask(KEY_RELEASE, FLGKEY_UP|FLGKEY_DOWN|FLGKEY_LEFT|FLGKEY_ENTER);
    ...
    */
    if (pDMenuInfo->uiSelectNum != NO_DEFAULT_RUN)
    {
        ...
        Ux_PostEvent(NVTEVT_KEY_LEFT, 2, NVTEVT_KEY_PRESS, pDMenuInfo->uiSelectNum);
    }
    else
    {
...
EVENT_ITEM(NVTEVT_KEY_UP,CalibrationMenu_OnKeyUp)
EVENT_ITEM(NVTEVT_KEY_DOWN,CalibrationMenu_OnKeyDown)
EVENT_ITEM(NVTEVT_KEY_ENTER,CalibrationMenu_OnKeyEnter)
#if (MODEKEY_FUNCTION == ENABLE)
EVENT_ITEM(NVTEVT_KEY_MODE,CalibrationMenu_OnKeyMode)
#endif
EVENT_ITEM(NVTEVT_KEY_LEFT,CalibrationMenu_OnKeyShutter)
        ...
        case NVTEVT_KEY_PRESS:
            if (UxCtrl_IsShow(&CalibrationMenuCtrl))
            {
                ...
                else
                {
                    ...
                    Ux_PostEvent(NVTEVT_SYSTEM_MODE, 1, PRIMARY_MODE_MOVIE); // TO DO: movie or photo?
                }
            }
            else
            {
