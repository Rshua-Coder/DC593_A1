...
EVENT_ITEM(NVTEVT_KEY_LEFT,MovieRecTestWnd_OnKeyShutter2)
...
INT32 MovieRecTestWnd_OnOpen(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    Ux_SendEvent(&CustomMovieObjCtrl, NVTEVT_EXE_MOVIESIZE,  1, MOVIE_SIZE_1080P);
    ...
    Ux_PostEvent(NVTEVT_KEY_LEFT, 1, NVTEVT_KEY_PRESS); // post shutter2 event to start recording
...
INT32 MovieRecTestWnd_OnKeyShutter2(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    else
    {
        ...
        g_bMovieRecording = FALSE;
        Ux_SendEvent(&CustomMovieObjCtrl,NVTEVT_EXE_MOVIE_REC_STOP,0);
        Delay_DelayMs(100); // make sure return to live view to avoid garbage screen
...
INT32 MovieRecTestWnd_OnMovieOneSec(VControl *pCtrl, UINT32 paramNum, UINT32 *paramArray)
{
    ...
    {
        Ux_PostEvent(NVTEVT_KEY_LEFT, 1, NVTEVT_KEY_PRESS); // post shutter2 event to stop recording
    }
    return NVTEVT_CONSUME;
}
