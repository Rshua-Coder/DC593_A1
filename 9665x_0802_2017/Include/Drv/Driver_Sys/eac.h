/**
    Embedded Audio Codec Driver Public Header File

    This file is the public header file for Embedded Audio Codec.
...
typedef enum
{
    ...
    EAC_CONFIG_AD_ALC_PEAK_SEL,     ///< Configure AD(Record) Auto Level Control (ALC) Peak value input Source form Left/Right or both. Use the enumeration "EAC_ALC_PEAK_SRC" as input parameter.
    EAC_CONFIG_AD_ALC_TARGET,       ///< Configure AD(Record) Auto Level Control (ALC) Target Level. Use the enumeration "EAC_ALC_TARGET" as input parameter.
    EAC_CONFIG_AD_ALC_MAXGAIN,      ///< Configure AD(Record) Auto Level Control (ALC) PGA Max Gain. Use the enumeration "EAC_ALC_MAXGAIN" as input parameter.
    EAC_CONFIG_AD_ALC_MINGAIN,      ///< Configure AD(Record) Auto Level Control (ALC) PGA Max Gain. Use the enumeration "EAC_ALC_MINGAIN" as input parameter.
    ...
    EAC_CONFIG_AD_ALC_NG_THD,       ///< Configure AD(Record) Auto Level Control (ALC) Noise Gate Threshold value. Please use the enumeration "EAC_NG_THRESHOLD" as input parameter.
    EAC_CONFIG_AD_ALC_NG_TARGET,    ///< Configure AD(Record) Auto Level Control (ALC) Noise Gate Target Level. Value range 0~15 which the Noise Gate target dB would be (ALC_TARGET - (NG_THD - INPUT_SIGNAL_LEVEL)*uiCfgValue).
    EAC_CONFIG_AD_ALC_NG_TRESO,     ///< Configure AD(Record) Auto Level Control (ALC) Noise Gate Time Resolution Basis during Attack/Decay time calculation inside Noise Gate state.
                                    ///< Please use "EAC_ALC_TRESO_BASIS" as input parameter. The default value is 10ms.
    ...
    EAC_CONFIG_AD_PGABOOST,         ///< Configure AD(Record) Analog Boost Gain. Please use EAC_PGABOOST_SEL as input parameter.
    ...
    EAC_CONFIG_AD_ZC_EN,            ///< Configure AD(Record) Zero Crossing Function Enable/Disable.
    EAC_CONFIG_AD_ZC_TIMEOUT,       ///< Configure AD(Record) Zero Crossing Function Timeout Value.
...
typedef enum
{
    ...
    EAC_CONFIG_DA_LINEOUT_GAIN,     ///< Configure DA(Playback) Analog Gain for Lineout path. NT96550 does not has lineout, so this would map to headphone path.
    ...
    EAC_CONFIG_DA_ZC_EN,            ///< Configure DA(Playback) Zero Crossing Function Enable/Disable.
    EAC_CONFIG_DA_SPKR_DRVING,      ///< Configure DA(Playback) Speaker Output Driving.
    EAC_CONFIG_DA_HP_DRVING,        ///< Configure DA(Playback) Headphone Output Driving.
...
/**
    Embedded Audio Codec Operation Mode

    This definition is used in eac_setMode() to specify the EAC operation mode.
*/
typedef enum
{
    EAC_MODE_DAC,                   ///< Digital to Analog Converter.
    EAC_MODE_ADC,                   ///< Analog to Digital Converter.

    ENUM_DUMMY4WORD(EAC_MODE)
}EAC_MODE;
...
typedef enum
{
    ...
    EAC_OUTPUT_HP,                  ///< DAC output to Headphone Stereo
    EAC_OUTPUT_HP_LEFT,             ///< DAC output to Headphone Left Channel
    EAC_OUTPUT_HP_RIGHT,            ///< DAC output to Headphone Right Channel

    EAC_OUTPUT_NONE,                ///< None of DAC output
    EAC_OUTPUT_ALL,                 ///< DAC output to Speaker&Headphone Stereo Channel
...
typedef enum
{
    EAC_DCCAN_TYPE_AUTO_RESET,      ///< DCCAN Type0: DCCAN would reset the current value after seting eac_setEnable(FALSE).
    EAC_DCCAN_TYPE_KEEP,            ///< DCCAN Type1: DCCAN would not reset the current value.
    EAC_DCCAN_TYPE_KEEP_OPTIMAL,    ///< DCCAN Type2: DCCAN would not reset the current value, and assign 96650 optimal DC initial value to controller.
    EAC_DCCAN_TYPE_RESET_OPTIMAL,   ///< DCCAN Type3: Assign 96650 optimal DC initial value to controller and DCCAN would reset the current value after seting eac_setEnable(FALSE)
...
/**
    ...
    But the larger average samples have good performance during very low input ftrequency such as 10 Hertz.
...
typedef enum
{
    ...
    EAC_DCCAN_RESO_2048_SAMPLES,    ///< DC Cancellation would take 2048 samples average as DC value. (Default value)
    EAC_DCCAN_RESO_4096_SAMPLES,    ///< DC Cancellation would take 4096 samples average as DC value.
    EAC_DCCAN_RESO_8192_SAMPLES,    ///< DC Cancellation would take 8192 samples average as DC value.
...
typedef enum
{
    ...
    EAC_PGABOOST_SEL_15DB,          ///< Enable  AD analog 15dB boost.
...
/**
    ...
    This definition is used in eac_setAdConfig(EAC_CONFIG_AD_ALC_MAXGAIN) to specify the ALC controllable Maximum PGA Gain.
*/
typedef enum
{
    ...
    EAC_ALC_MAXGAIN_N4P5_DB,        ///< ALC controllable Maximum Gain as  -5.5dB.
...
/**
    ...
    This definition is used in eac_setAdConfig(EAC_CONFIG_AD_ALC_MINGAIN) to specify the ALC controllable Minimum PGA Gain.
*/
typedef enum
{
    EAC_ALC_MINGAIN_N21P0_DB,       ///< ALC controllable Minimum Gain as -21.5dB.
    EAC_ALC_MINGAIN_N15P0_DB,       ///< ALC controllable Minimum Gain as -15.0dB.
    EAC_ALC_MINGAIN_N9P0_DB,        ///< ALC controllable Minimum Gain as  -9.0dB.
    EAC_ALC_MINGAIN_N3P0_DB,        ///< ALC controllable Minimum Gain as  +3.0dB.
    EAC_ALC_MINGAIN_P3P0_DB,        ///< ALC controllable Minimum Gain as  +3.0dB.
    EAC_ALC_MINGAIN_P9P0_DB,        ///< ALC controllable Minimum Gain as  +9.0dB.
    EAC_ALC_MINGAIN_P15P0_DB,       ///< ALC controllable Minimum Gain as +15.0dB.
    EAC_ALC_MINGAIN_P21P0_DB,       ///< ALC controllable Minimum Gain as +21.0dB.

    EAC_ALC_MINGAIN_MAX = 0x7,
...
/**
    ...
    The NT96650 Lowest Decay time is 800us and lowest Attack time is 1.6ms.
...
//
#define EAC_DCCAN_TYPE0     EAC_DCCAN_TYPE_AUTO_RESET
#define EAC_DCCAN_TYPE1     EAC_DCCAN_TYPE_KEEP
#define EAC_DCCAN_TYPE2     EAC_DCCAN_TYPE_KEEP_OPTIMAL
#define EAC_DCCAN_TYPE3     EAC_DCCAN_TYPE_RESET_OPTIMAL
...
extern void     eac_setEnable(BOOL bEnable);
extern BOOL     eac_getEnable(void);
extern void     eac_setMode(EAC_MODE Mode);
extern EAC_MODE eac_getMode(void);
