...
typedef struct _LDINFO
{
    ...
    UINT32 LdBlockCount;///< [0xa0] Ld size (n blocks) (4) ------ w by Ld
    UINT32 FWResvSize;  ///< [0xa4] FW reserved size (4) ------ w by Ld
    UINT32 Resv[6];     ///< [0xa8~0xc0] (4*6) ------------ reserved for project Ld
}
LDINFO;
...
typedef struct _STRGINFO
{
    ...
    UINT32 StrgInfo[6]; ///< [0x118~0x130] nand partition (4*6)
                        ///< [0x118] FWRSV offset (4) (must be 0)
                        ///< [0x11c] FWRSV size (4)(w by FW, but will compare to LDINFO.FWResvSize)
                        ///< [0x120] FAT offset (4)
                        ///< [0x124] FAT size (4)
                        ///< [0x128] PSTORE offset (4)
                        ///< [0x12c] PSTORE size (4)
    UINT32 Resv[4];     ///< [0x130~0x140] (4*4) ---------- reserved for project nand
}
STRGINFO;
...
@$(BFC) c lz $(BIN_R) tmp 1 0x310 0x68 0x78 $(NAND_BLK_SIZE)
*/
