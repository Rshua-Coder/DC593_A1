...
#define NVTBP_CHADDR_LEN    16
#define NVTBP_SNAME_LEN     64
#define NVTBP_FILE_LEN     128
#define NVTBP_VEND_LEN     (312 + 32)

typedef struct _NVT_BOOTP {
    ...
    unsigned int     bp_ciaddr;         /* client IP address */
    unsigned int     bp_yiaddr;         /* 'your' IP address */
    unsigned int     bp_siaddr;         /* server IP address */
    unsigned int     bp_giaddr;         /* gateway IP address */
    unsigned char    bp_chaddr[NVTBP_CHADDR_LEN];  /* client hardware address */
    char             bp_sname[NVTBP_SNAME_LEN];    /* server host name */
    char             bp_file[NVTBP_FILE_LEN];  /* boot file name */
    unsigned char    bp_vend[NVTBP_VEND_LEN];  /* vendor-specific area */
    /* note that bp_vend can be longer, extending to end of packet. */
} NVT_BOOTP,*PNVT_BOOTP;
...
typedef enum
{
    ...
    NET_DHCP_CB_TYPE_CLI_KEEP_RESULT,
...
/**
     Reboot the DSC as a DHCP client.

     The DSC will boot TCP/IP  by given DHCP information and send an send DHCP request to DHCP Server .

     @param[in] pName             points to a string of host name,and it must be end with 0.
     @param[in] pFunc             points to a function with type, NET_DHCP_CB_FUNC.
     @param[in] wlan0_bootp_last     pointer of last DHCP information.
     ...
     @note:
         The default name is "nvtdsc01cli_dep" if pName is null.
*/
NET_DHCP_RET Dhcp_Client_ReBoot(char *pName, NET_DHCP_CB_FUNC pFunc, NVT_BOOTP *wlan0_bootp_last);
