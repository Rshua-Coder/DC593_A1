...
/**
     Apply a image with Nova Light Effect(HDR).

     This function will apply a image with Nova Light Effect(similar with HDR effect),

     @param[in] p_pbxEditParam description.

     @note: p_pbxEditParam->paramNum -> the parameter number should be 1.\n
            p_pbxEditParam->param[0] -> PBXEDIT_NLIGHT_LEVEL_SEL, the Nlight effect level
     ...
     @code
     {
         ...
         EditParam.workBufSize = WORKBUFF_SIZE;
         EditParam.paramNum = 1; // should be 1
         Param[0] = nlightLvl;   // set the novalight level
         EditParam.param = Param;
         // set the function pointer to EDIT_Nlight
         PBXEdit_SetFunc(EDIT_Nlight);
         // run the Nlight function
     ...
     @endcode
*/
extern ER    EDIT_Nlight(PPBX_EDIT_PARAM p_pbxEditParam);
