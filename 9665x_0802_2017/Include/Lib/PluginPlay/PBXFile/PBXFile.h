...
/**
    Type defintion struture of playback file object.

    Type defintion struture of playback file object functions.
*/
typedef struct
{
    BOOL   (*WaitInit)(void);                                                             ///< Wait file system init finish.
    UINT32 (*GetInfo)(PBX_FILEINFO  WhichInfo, UINT32 parm1);                             ///< Get file info.
    ER     (*GetTime)(char* szFileName, UINT32  creDateTime[6] , UINT32  modDateTime[6]); ///< Get a file time.
    ER     (*SetTime)(char* szFileName, UINT32  creDateTime[6] , UINT32  modDateTime[6]); ///< Set a file time.
    ER     (*Access)(PPBXFile_Access_Info  pfileAccess);                                  ///< Read/Write a file data with specific position.
    UINT64 (*GetFreeSpace)(void);                                                         ///< Get the current storage free space of filesystem.
} PBX_FILE_OBJ, *PPBX_FILE_OBJ;


/**
     Set the playback file object.
*/
extern void PBXFile_SetObject(PPBX_FILE_OBJ  pPBXFileObj);
/**
     Get the playback default file object.
*/
extern PPBX_FILE_OBJ PBXFile_GetDefObject(void);
#endif
