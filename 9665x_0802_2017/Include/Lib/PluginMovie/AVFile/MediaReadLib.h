...
#define MEDIA_FILFFORMAT_TOTAL  3      ///< Totoal of file format type
//@}
...
typedef struct
{
    ...
    ER (*cbReadFile)(UINT8 *type, UINT32 pos, UINT32 size, UINT32 addr);            ///< file read callback function
...
typedef struct
{
    ...
    UINT32 fre1_pos;///< [out] file position of user data
...
typedef struct
{
    ...
    UINT32 uiPriority;                ///< Value : 0x00000000
    UINT32 uiLanguage;                ///< Value : 0x00000000
    UINT32 uiInitialFrames;           ///< Value : 0x00000000
...
typedef struct
{
    ...
    UINT32 uiPlanes;                  ///< The number of planes
    UINT32 uiBitCount;                ///< The number of bits-per-pixel
...
typedef struct
{
    UINT32 FOURCC_ID;                 ///< Four-Character Code ID
    UINT32 uiSize;                    ///< Info Size
    UINT32 uiFormatTag;               ///< Waveform-audio format type (0x00000001)
    UINT32 uiChannels;                ///< Number of channels
    UINT32 uiSamplesPerSec;           ///< Sample rate in samples per second (Hz)
    UINT32 uiAvgBytesPerSec;          ///< Average of bytes per second
    UINT32 uiBlockAlign;              ///< Block alignment in bytes
    UINT32 uiBitsPerSample;           ///< Bits per sample
    UINT32 uicbSize;                  ///< cb Size
